...
I wanted a simple, automated way to cluster the U-matrix that comes out of a SOM. That seemed doable, but then I started looking at what Python has to offer. There are several libraries, and what surprised me was not so much that they work differently under the hood, but that the clustering results and quality can vary quite a bit between them. I ran some experiments on a few datasets from the Fundamental Clustering Problems Suite (FCPS); results are in [`tuning/results/`](tuning/results/), and the scores across backends were different enough to care about. So while building this I also tried to give it a **pluggable backend structure** with consistent U-matrix, P-matrix, and U\* helpers, so at least the topology layer is the same regardless of who trained the weights.
